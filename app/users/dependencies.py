...
async def get_current_user(token=Depends(get_token)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Куки истек")
    except JWTError:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Неверный формат куки")
